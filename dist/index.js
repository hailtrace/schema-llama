      ...
      case 'attemptCast':
        {
          if (options[key].constructor !== Boolean) {
            throw new _errors.TypeError(`attemptCast option must be a Boolean.`);
          }
          break;
        }
      case 'strict':
        {
          if (options[key].constructor !== Boolean) {
            throw new _errors.TypeError(`strict option must be a Boolean`);
          }
          break;
        }
      case 'get':
        {
          if (!options[key] || options[key].constructor !== Function) {
            throw new _errors.TypeError(`get() hook must be a function.`);
          }
          break;
        }
      case 'set':
        {
          if (!options[key] || options[key].constructor !== Function) {
            throw new _errors.TypeError(`set() hook must be a function.`);
          }
          break;
        }
      default:
        {
          throw new _errors.TypeError(`Option key <${key}> is not a valid option.`);
          break;
        }
    }
  }
...
const classFactory = function (schema) {
  ...
  return (...args) => {
    const [options, ParentClass] = args.length > 2 ? (() => {
      throw (0, _errors.Error)('Invalid argument length for Schema(options)([options][,ParentClass])');
    })() : [args[0] || {}, args[1] || undefined];
    if (options && typeof options !== 'object') throw (0, _errors.TypeError)('You must provivide a valid options object.');
